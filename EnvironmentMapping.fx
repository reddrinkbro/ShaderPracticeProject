...
// Pass 0
//--------------------------------------------------------------//
string EnvironmentMapping_Pass_0_Model : ModelData = ".\\sphere1.x";
...
<
   string ResourceName = "C:\\Program Files (x86)\\AMD\\RenderMonkey 1.82\\Examples\\Media\\Textures\\fieldstone_SM.tga";
>;
...
float4 EnvironmentMapping_Pass_0_Pixel_Shader_ps_main(PS_INPUT Input) : COLOR
{
   ...
   TBN = transpose(TBN);
   float3 worldNoraml = mul(TBN,tangentNormal);
   float4 albedo = tex2D(DiffuseSampler, Input.mUV);
   float3 lightDir = normalize(Input.mLightDir);
   float3 diffuse = saturate(dot(worldNoraml,-lightDir));
   ...
   if(diffuse.x > 0)
   {
      float3 reflection = reflect(lightDir,worldNoraml);
   ...
   float3 viewReflect = reflect(viewDir,worldNoraml);
